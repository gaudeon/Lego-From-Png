...
# ts/003_when_loading_brick_information.t - test loading lego brick information
...
sub should_load_all_color_constants {
   ...
   mostly_like($colors, $expected_colors, 'should load all color constants');
}
...
sub should_load_lego_dimensions {
   ...
   mostly_like($object->lego_dimensions->{'metric'}, $expected_dim_in_millimeters, 'should load lego brick dimensions in millimeters by default');

   mostly_like($object->lego_dimensions->{'imperial'}, $expected_dim_in_inches, 'should load lego brick dimensions in inches when requested');
}
...
sub should_load_all_brick_dimensions {
    ...
    mostly_like($lengths, $expected_lengths, 'should load all brick lengths');
}
